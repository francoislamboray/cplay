...
$query = 'SELECT *, games.id AS gid FROM games LEFT JOIN games_type ON games.type_id = games_type.id INNER JOIN users_game ON games.id = users_game.game_id WHERE users_game.user_id = 1';
...
<div class="grid-x games">
    <?php foreach( $games as $key => $game ){
        if( !empty( $_GET['search'] ) && stripos( $game['name'], $_GET['search'] ) === false ){
        continue;
        }
        ?>
